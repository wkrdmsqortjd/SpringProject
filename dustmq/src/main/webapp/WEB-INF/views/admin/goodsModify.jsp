...
<body>
	...
	<script>
		...
		$(document).ready(function(){
			...
			/* 수정 페이지이기 때문에 DB에 저장된 데이터가 출력되어야함. 하지만 이미 datepicker가 적용되어 input태그에 value를 추가해도 출력 x */			
			/* 캘린더 */
			$(function() {
				let publeYear = '${goodsInfo.publeYear}';   // DB에서 가져온 상품 출판일을 변수에 초기화
				let puble_length = publeYear.indexOf(" ");	// yyyy-mm-dd 00:00:00
				publeYear = publeYear.substring(0, puble_length);	// yyyy-mm-dd
				$("input[name='publeYear']").datepicker(config);
				$("input[name='publeYear']").datepicker('setDate', publeYear);	// 가져온 변수에 새롭게 datepicker로 날짜 세팅
			});				
		...
		$.getJSON("/getAttachList", {bookId : bookId}, function(arr){
			...
			/* 삭제 버튼과 출력된 이미지 정보가 담긴 input태그 코드 추가 */
	...
	<script>
	<!-- 사용자 선택에 따라 분류들이 변경되도록 구현, 렌더링 될 때 실행될 코드와 구분하기위해 새로운 script태그 생성 -->
	/* 카테고리 */
	let cateList = JSON.parse('${cateList}');
	...
	$("#deleteBtn").on("click", function(e){
		...
		function deleteFile(){
			...
			// 이미지 태그와 이미지 정보가 담긴 <input>태그를 감싸는 div태그인 #result_card를 제거
			$("#result_card").remove();
